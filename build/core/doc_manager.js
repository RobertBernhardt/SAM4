...
function createDocInFolder_(title, customFolderId) {
...
// ─── Quest State Docs ───────────────────────────────────────
function createQuestStateDoc_(questId, description) {
 ...
 */
function ensureQuestStateDoc_(questId, description) {
...
function createExperienceDoc_(agentId) {
 ...
 * Returns the doc URL (existing or newly created).
 */
function ensureExperienceDoc_(agentId) {
...
function readDocContent(docUrl) {
...
function appendToDoc(docUrl, content) {
    const docId = extractDocId_(docUrl);
    if (!docId)
        return 'ERROR: Invalid doc URL';
    try {
        const doc = DocumentApp.openById(docId);
        const body = doc.getBody();
...
function overwriteDocContent(docUrl, newContent) {
    const docId = extractDocId_(docUrl);
    if (!docId)
        return 'ERROR: Invalid doc URL';
    try {
