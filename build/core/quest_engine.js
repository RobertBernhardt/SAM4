...
function queueOutboxMessage_(questId, message, bot, metadata) {
...
function processQuests() {
    ...
    const questPrompt = [
        ...
        ``,
        `REPORT TO CREATOR:`,
        `[Concise summary including blockers]`,
    ].join('\n');
...
function markLatestDeliveredAsReplied_() {
    const sheet = getOutboxSheet_();
    const data = sheet.getDataRange().getValues();
    for (let i = data.length - 1; i > 0; i--) {
        if (String(data[i][3]).trim() === 'DELIVERED') {
...
function getLatestDeliveredQuestId_() {
    const sheet = getOutboxSheet_();
    const data = sheet.getDataRange().getValues();
    for (let i = data.length - 1; i > 0; i--) {
        if (String(data[i][3]).trim() === 'DELIVERED') {
 ...
 */
function parseNLNewQuest(userText, updateId) {
    const contextMessage = [
...
function updateTelemetry() {
    try {
        ...
        if (manifestSheet) {
            ...
            // Ensure headers
            if (String(manifestData[0][7] || '') !== 'lifetime_usage')
                manifestSheet.getRange(1, 8).setValue('lifetime_usage');
            if (String(manifestData[0][9] || '') !== 'total_invocations')
                manifestSheet.getRange(1, 10).setValue('total_invocations');
        ...
        if (toolSheet) {
            ...
            if (String(toolData[0][5] || '') !== 'lifetime_usage')
                toolSheet.getRange(1, 6).setValue('lifetime_usage');
...
function triggerExperienceReview() {
    const ss = SpreadsheetApp.openById(getSamSheetId());
