...
interface ReferencePayload {
    textRefs: string;
    imageRefs: GeminiPart[];
}

function getReferencesPayload(algoId: string): ReferencePayload {
    const payload: ReferencePayload = { textRefs: '', imageRefs: [] };
 ...
 */
function getQuestReferencesPayload(questId: string): ReferencePayload {
    const payload: ReferencePayload = { textRefs: '', imageRefs: [] };
    ...
    for (let i = 1; i < data.length; i++) {
        const row = data[i];
        if (String(row[0]).trim() !== questId) continue;
